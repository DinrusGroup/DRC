...
/*
  ...
  данные.  This version of the library supports only one compression method
  ...
  (providing more output space) before each call.
     ...
     The compressed данные format used by default by the in-memory functions is
...
struct z_stream
{
    ...
    void*      opaque;  /* private данные object passed to zalloc and zfree */

    int    data_type;  /* best guess about the данные type: binary or text */
    uint   adler;      /* adler32 value of the uncompressed данные */
...
*/
struct gz_header {
    int     text;       /* true if compressed данные believed to be text */
    ulong   time;       /* modification time */
    int     xflags;     /* extra flags (not used when writing a gzip file) */
    int     os;         /* operating system */
    byte    *extra;     /* pointer to extra field or Z_NULL if none */
    uint    extra_len;  /* extra field длина (valid if extra != Z_NULL) */
...
/* 
   ...
   all (the input данные is simply copied a block at a time).
...
/*
    deflate compresses as much данные as possible, and stops when the input
  ...
  decide how much данные to accumualte before producing output, in order to
  maximize compression.

    If the parameter flush is set to Z_SYNC_FLUSH, all pending output is
  flushed to the output buffer and the output is aligned on a byte boundary, so
  that the decompressor can get all input данные available so far. (In particular
  ...
  more input данные, until it returns with Z_STREAM_END or an error. After
  ...
  the input данные type (Z_BINARY or Z_TEXT). In doubt, the данные is considered
...
/* 
   ...
   compression method from the zlib header and allocates all данные structures
...
/*
    inflate decompresses as much данные as possible, and stops when the input
  ...
  - Provide more output starting at next_out and update next_out and avail_out
    ...
    is no more input данные or no more space in the output buffer (see below
  ...
  the end of that block, or when it runs out of данные.
  ...
  uncompressed данные from that block has been written to strm->next_out.  The
  ...
  deflate данные.  The header type is detected automatically.  Any information
  ...
  or more output produced), Z_STREAM_END if the end of the compressed данные has
  been reached and all uncompressed output has been produced, Z_NEED_DICT if a
  preset dictionary is needed at this point, Z_DATA_ERROR if the input данные was
...
/*
   ...
   determines the window size. deflate() will then generate raw deflate данные
   with no zlib header or trailer, and will not compute an adler32 check value.

     windowBits can also be greater than 15 for optional gzip encoding. Add
   16 to windowBits to write a simple gzip header and trailer around the
   compressed данные instead of a zlib wrapper. The gzip header will have no
   file name, no extra данные, no comment, no modification time (set to zero),
   ...
   string match), or Z_RLE to limit match distances to one (run-длина
   encoding). Filtered данные consists mostly of small values with a somewhat
   random distribution. In this case, the compression algorithm is tuned to
   compress them better. The effect of Z_FILTERED is to force more Huffman
   coding and less string matching; it is somewhat intermediate between
   Z_DEFAULT and Z_HUFFMAN_ONLY. Z_RLE is designed to be almost as fast as
   Z_HUFFMAN_ONLY, but give better compression for PNG image данные. The strategy
...
/*
   ...
   to be encountered later in the данные to be compressed, with the most commonly
   used strings preferably put towards the end of the dictionary. Using a
   dictionary is most useful when the данные to be compressed is short and can be
   predicted with good accuracy; the данные can then be compressed better than
   with the default empty dictionary.

     Depending on the size of the compression данные structures selected by
...
/*
   ...
   данные with a filter. The streams that will be discarded should then be freed
...
/*
   ...
   extra_len contains the actual extra field длина, and extra contains the
   extra field, or that field truncated if extra_max is less than extra_len.
   If name is not Z_NULL, then up to name_max characters are written there,
   terminated with a zero unless the длина is greater than name_max.  If
   comment is not Z_NULL, then up to comm_max characters are written there,
   terminated with a zero unless the длина is greater than comm_max.  When
...
/*
   ...
   used to switch between compression and straight copy of the input данные, or
   to switch to a different kind of input данные requiring a different
...
/*
   ...
   specific input данные.  Read the deflate.c source code for the meaning of the
...
/* 
    Skips invalid compressed данные until a full flush point (see above the
  ...
  indicates where valid compressed данные was found. In the error case, the
  application may repeatedly call inflateSync, providing more input each time,
  until success or end of the input данные.
*/
...
/*
   ...
   uncompressed данные, or until it encounters an error.  The function's
   parameters and return types are defined above in the in_func and out_func
   typedefs.  inflateBack() will call in(in_desc, &buf) which should return the
   number of bytes of provided input, and a pointer to that input in buf.  If
   there is no input available, in() must return zero--buf is ignored in that
   case--and inflateBack() will return a buffer error.  inflateBack() will call
   out(out_desc, buf, len) to write the uncompressed данные buf[0..len-1].  out()
...
/* Return flags indicating compile-time options.
    ...
    The sprintf variant used by gzprintf (zero is best):
     ...
     25: 0 = *nprintf, 1 = *эхо -- 1 means gzprintf() not secure!
     26: 0 = returns value, 1 = void -- 1 means inferred string длина returned
...
/*
   ...
   the byte длина of the source buffer. Upon entry, destLen is the total
...
/*
   ...
   длина of the source buffer. Upon entry, destLen is the total size of the
...
/*
   ...
   entire uncompressed данные. (The size of the uncompressed данные must have
   ...
   enough memory, Z_BUF_ERROR if there was not enough room in the output
   buffer, or Z_DATA_ERROR if the input данные was corrupted or incomplete.
*/
...
/*
   ...
   Huffman only compression as in "wb1h", or 'R' for run-длина encoding
...
/* 
   ...
   uncompressed данные stream. The whence parameter is defined as in lseek(2);
...
/*
   ...
   much faster. Usage example:
     ...
     while (read_buffer(buffer, длина) != EOF) {
       adler = adler32(adler, buffer, длина);
     }
