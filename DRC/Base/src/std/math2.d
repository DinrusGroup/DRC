...
module std.math2;
private import std.math, std.string;
...
real abs(real n)
{
	// just let the compiler хэндл it
...
real atof(char[] s)
{
	...
	if (eneg)
		e = -e;
	результат *= std.math.pow(hex ? 2.0L : 10.0L, cast(real)e);
done:	
	return neg ? -результат : результат;
}
