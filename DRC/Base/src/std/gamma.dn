...
private real gammaStirling(real x)
{
    ...
    real w = 1.0L / x;
    real y = std.math.exp(x);
    if (x > 1024.0L)
    {
	// For large x, use rational coefficients from the analytical expansion.
	w = poly(w, LargeStirlingCoeffs);
	// Avoid overflow in pow()
	real v = std.math.pow( x, 0.5L * x - 0.25L );
	y = v * (v / y);
    }
    else
    {
	w = 1.0L + w * poly(w, SmallStirlingCoeffs);
	y = std.math.pow( x, x - 0.5L ) / y;
    }
    y = SQRT2PI * y * w;
    return  y;
}
...
real tgamma(real x)
{
	...
	if ( x < 0.0L )
	{
	    ...
	    z = q * std.math.sin( PI * z );
	    z = std.math.fabs(z) * gammaStirling(q);
	    if ( z <= PI / real.max )
...
real lgamma(real x)
{
	...
	while ( x < 2.0L )
	{
		...
		return std.math.log( std.math.fabs(q) );
	    }			
	    z /= nx +  f;
	    nx += 1.0L;
	    x = nx + f;
    ...
    q = ( x - 0.5L ) * std.math.log(x) - x + LOGSQRT2PI;
...
unittest
{
    ...
    assert(std.math.isnan(lgamma(real.nan)));
