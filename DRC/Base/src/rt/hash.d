...
extern (C) hash_t rt_hash_combine( hash_t val1, hash_t val2 ){
    static if (is(hash_t==uint)||is(hash_t==int)){
        ...
        static assert(0,"неподдерживаемый тип для hash_t: "~hash_t.stringof);
    }
}
...
hash_t murmurHashAligned2 ( void *key, size_t len, size_t seed )
{
    const uint m = 0x5bd1e995;
    const int r = 24;
...
uint murmurHashNeutral2 ( void * key, uint len, uint seed )
{
    const uint m = 0x5bd1e995;
    const int r = 24;
