...
class Интерпретатор : Визитёр
{
  ...
  {
    override Итог копируй(){return пусто;}
  }
  ...
  {
    // We cache this результат so that we don'т blindly try в reevaluate
    // functions that can'т be evaluated at compile время
  ...
  static бул булев(Выражение в, бул значение)
  {
    switch(в.вид)
    {
      alias ВидУзла ВУ;
...
override
{
  ...
  { assert(0, "интерпретируется сломанное АСД"); return пусто; }
...
override
{
  ...
  T посети(ТипКлассОснова т)
  {
    return т;
  }

  T посети(ТКонст т) // D2.0
  {
    return т;
  }

  T посети(ТИнвариант т) // D2.0
  {
    return т;
  }
} // override
...
override
{
  ...
  N посети(Параметры у)
  {
    return у;
  }

  N посети(ПараметрАлиасШаблона у)
  {
    return у;
  }

  N посети(ПараметрТипаШаблона у)
  {
    return у;
  }

  N посети(ПараметрЭтотШаблона у) // D2.0
  {
    return у;
  }

  N посети(ПараметрШаблонЗначения у)
  {
    return у;
  }

  N посети(ПараметрКортежШаблона у)
  {
    return у;
  }

  N посети(ПараметрыШаблона у)
  {
    return у;
  }

  N посети(АргументыШаблона у)
  {
    return у;
  }
} // override
