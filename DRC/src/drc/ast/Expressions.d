...
{
  Выражение лв; /// Левая сторона выражение.
  Выражение пв; /// Правая сторона выражение.
  Сема* лекс;
...
{
  Выражение в; // TODO: переименовать 'в' в 'следщ', 'унарное', 'выр' или 'подвыр' и т.д..
  this(Выражение в)
  {
    добавьОтпрыск(в);
    this.в = в;
...
class ВыражениеНов : /*Унарное*/Выражение
{
  Выражение[] новАрги;
  УзелТипа тип;
  Выражение[] кторАрги;
...
class ВыражениеНовАнонКласс : /*Унарное*/Выражение
{
  Выражение[] новАрги;
...
class ВыражениеКаст : УнарноеВыражение
{
  ...
  {
    добавьОтпрыск(тип); // Добавить тип перед super().
    super(в);
    mixin(установить_вид);
    this.тип = тип;
  }
...
class ВыражениеСрез : УнарноеВыражение
{
  ...
  this(Выражение в, Выражение левый, Выражение правый)
  {
    super(в);
    mixin(установить_вид);
    assert(левый ? (правый !is пусто) : правый is пусто);
    if (левый)
...
/// Модуль Масштаб оператор: '.' (ВыражениеИдентификатор|ВыражениеЭкземплярШаблона)
...
class ВыражениеСпецСема : Выражение
{
  ...
  Выражение значение; /// Выражение, созданное в семантической фазе.
...
{
  ЦелВыражение значение; /// ЦелВыражение типа бул.

  this(бул значение)
  {
    mixin(установить_вид);
    // Некоторое семантическое вычисление в данном месте.
...
class ЦелВыражение : Выражение
{
  ...
  {
    // Некоторое семантическое вычисление в данном месте.
    auto тип = Типы.Цел; // Должно быть самым общим случаем.
    switch (сема.вид)
    {
    // case TOK.Цел32:
    //   тип = Типы.Цел; break;
...
class ВыражениеРеал : Выражение
{
  ...
  {
    // Некоторое семантическое вычисление в данном месте.
    auto тип = Типы.Дво; // Самый общий случай?
    switch (сема.вид)
    {
    case TOK.Плав32:
      тип = Типы.Плав; break;
...
/// Это выражение содержит комплексное число.
/// Создаётся только на семантической фазе.
...
{
  ЦелВыражение значение; // ЦелВыражение типа Сим/Шим/Дим.
//  дим символ; // Земенено значением.
  this(дим символ)
  {
    mixin(установить_вид);
//    this.символ = символ;
...
{
  ббайт[] ткт;   /// Текстовые данные.
  Тип типСим; /// Символьный тип текста.
  ...
  this(ббайт[] ткт, Тип типСим)
  {
    ...
    this.ткт = ткт;
    this.типСим = типСим;
    this.тип = new ТипСМассив(типСим, ткт.length);
  }
  ...
  /// Возвращает ткст, исключая завершающий 0.
  ткст дайТекст()
  {
    // TODO: преобразовать в ткст, если типСим !is Типы.Сим.
...
class ВыражениеЛитералАМассива : Выражение
{
  ...
  this(Выражение[] ключи, Выражение[] значения)
  {
    ...
    mixin(установить_вид);
    foreach (i, ключ; ключи)
      добавьОтпрыски([ключ, значения[i]]);
    this.ключи = ключи;
    this.значения = значения;
  }
  mixin(методКопирования);
...
class ВыражениеИницМассива : Выражение
{
  ...
  this(Выражение[] ключи, Выражение[] значения)
  {
    ...
    foreach (i, ключ; ключи)
    {
      добавьОпцОтпрыск(ключ); // Ключ необязателен в ИнициализаторыМассива.
...
{
  Выражение e2; /// Выражение в скобках: в [ e2 ]
...
class ВыражениеАсмРегистр : Выражение
{
  ...
  цел число; // ST(0) - ST(7) или FS:0, FS:4, FS:8
