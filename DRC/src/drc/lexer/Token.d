...
/// A Сема - из_ цепочка символов, формируемая лексическим анализатором.
struct Сема
{ /// Флаги, устанавливаемые Лексером.
  ...
  /// Возвращает да, если из_ сема, внутри которой могут быть символы новой строки.
  ///
  ...
  /// Возвращает да, если из_ сема-ключевое слово.
  бул кслово_ли()
  {
    return НачалоКС <= вид && вид <= КонецКС;
  }

  /// Возвращает да, если из_ сема интегрального типа.
  бул интегральныйТип_ли()
  {
    return НачалоИнтегральногоТипа <= вид && вид <= КонецИнтегральногоТипа;
  }

  /// Возвращает да, если из_ сема пробела.
  бул пробел_ли()
  {
...
version(D2)
{
  ...
  Положение дайПоложение(бул реальноеПоложение)()
  {
    ...
    else
    {
      ...
      auto номСтр   = search_t.нс.oriLineNum - search_t.нс.setLineNum;
    }
