...
//     this.нс = нс;
  ...
  проц  закончиОсобуюСему(ref Сема t)
  {
    ...
    case TOK.ПОСТАВЩИК:
      ...
      break;
    case TOK.ВЕРСИЯ:
      t.бцел_ = ВЕРСИЯ_МАЖОР*1000 + ВЕРСИЯ_МИНОР;
      break;
    default:
      assert(0);
    }
...
//         this.нс = &t;
      ...
      {
        if (c == 'r' && p[1] == '"' && ++p)
          return сканируйСыройСтроковыйЛитерал(t);
        if (c == 'x' && p[1] == '"')
          return сканируйСтроковыйГексЛитерал(t);
      version(D2)
      {
        if (c == 'q' && p[1] == '"')
          return сканируйЛитералОграниченнойСтроки(t);
        if (c == 'q' && p[1] == '{')
          return сканируйСтроковыйЛитералСемы(t);
      }
        // Scan identifier.
      Lidentifier:
        do
      ...
      if (c == '/')
      {
        ...
        case '=':
          ...
          return;
        case '+':
          return сканируйГнездовойКомментарий(t);
        case '*':
          return сканируйБлочныйКомментарий(t);
        case '/':
          while (!конецСтроки_ли(++p))
            аски_ли(*p) || раскодируйЮ8();
      ...
      case '`':
        return сканируйСыройСтроковыйЛитерал(t);
      case '"':
        return сканируйНормальныйСтроковыйЛитерал(t);
      case '\\':
        ткст буфер;
        do
        {
          бул isBinary;
          c = сканируйИскейпПоследовательность(isBinary);
      ...
      Lcommon2:
        ...
        return;
      case '#':
        return сканируйОсобуйПоследовательностьСем(t);
      default:
      }
  ...
  public проц  сканируй_(ref Сема t)
  in
  {
    assert(текст.ptr <= p && p < конец);
  }
...
//       this.нс = &t;
  ...
  L1character:
    ...
    Lidentifier:
      ...
      else if (t.вид == TOK.КФ)
      {
        ...
        assert(0, "неожиданный тип семы: " ~ Сема.вТкст(t.вид));
      return;
    }
  ...
  проц  сканируйНормальныйСтроковыйЛитерал(ref Сема t)
  {
    assert(*p == '"');
    auto tokenLineNum = номСтр;
    auto tokenLineBegin = началоСтроки;
  ...
  проц  сканируйСимвольныйЛитерал(ref Сема t)
  {
    ...
    case '\\':
      бул notused;
      t.дим_ = сканируйИскейпПоследовательность(notused);
      break;
    case '\'':
  ...
  проц  сканируйСтроковыйГексЛитерал(ref Сема t)
  {
    assert(p[0] == 'x' && p[1] == '"');
    t.вид = TOK.Ткст;
    ...
    while (1)
    {
      ...
      case '"':
        if (n & 1)
          ошибка(tokenLineNum, tokenLineBegin, t.старт, ИДС.НечётноеЧислоЦифрВГексСтроке);
        ++p;
        t.pf = сканируйПостфикс();
      Lreturn:
      ...
      default:
        ...
        {
          ошибка(tokenLineNum, tokenLineBegin, t.старт, ИДС.НеоконченнаяГексСтрока);
          t.pf = 0;
          goto Lreturn;
        }
        else
        {
          ...
          ошибка(errorAt, ИДС.НеГексСимВГексСтроке, cast(дим)c);
        }
      }
    }
    assert(0);
...
version(DDoc)
{
  ...
  проц  сканируйЛитералОграниченнойСтроки(ref Сема t);
  /// Scans a сема ткст literal.
  ///
  /// TokenStringLiteral := "q{" Сема* "}"
  проц  сканируйСтроковыйЛитералСемы(ref Сема t);
}
else
version(D2)
{
  ...
  проц  сканируйСтроковыйЛитералСемы(ref Сема t)
  {
    ...
    {
      // Присвоить в буфер before сканируйПостфикс().
      t.конец = p;
  ...
  дим сканируйИскейпПоследовательность(ref бул isBinary)
  out(результат)
  ...
  body
  {
    ...
    auto началоПоследовательности = p; // Used for ошибка reporting.
        ...
        if (гекс_ли(*p))
        {
          ...
          if (--цифры == 0)
          {
            ...
            ошибка(началоПоследовательности, ИДС.НевернаяИскейпПоследовательностьЮникод,
                  началоПоследовательности[0..p-началоПоследовательности]);
            break;
          }
          continue;
        }
        ...
        if (буква_ли(*++p))
        {
          ...
          if (*p == ';')
          {
            ...
            if (c != 0xFFFF)
              ...
              ошибка(началоПоследовательности, ИДС.НеопределённаяСущностьГТЯР, началоПоследовательности[0 .. p - началоПоследовательности]);
          }
          else
            ошибка(началоПоследовательности, ИДС.НеоконченнаяСущностьГТЯР, началоПоследовательности[0 .. p - началоПоследовательности]);
        }
        else
  ...
  ///
  /// $(PRE
  /// IntegerLiteral := (Dec|Гекс|Bin|Oct)Суффикс?
  /// Dec := (0|[1-9][0-9_]*)
  ...
  LscanInteger:
    ...
    Lenter_loop_int:
      ...
      // Overflow: пропусти following цифры.
      overflow = да;
      while (цифра_ли(*++p)) {}
      break;
    }
  ...
  LscanHex:
    ...
    case '.':
      if (p[1] == '.')
        break;
    case 'p', 'P':
      return сканируйГексРеал(t);
    default:
    }

    if (цифры == 0 || цифры > 16)
      ошибка(t.старт, цифры == 0 ? ИДС.НетЦифрВГексЧисле : ИДС.ПереполнениеГексЧисла);
  ...
  LscanBinary:
    ...
    if (цифры == 0 || цифры > 64)
      ошибка(t.старт, цифры == 0 ? ИДС.НетЦифрВБинЧисле : ИДС.ПереполнениеБинЧисла);
  ...
  LscanOctal:
    ...
    if (hasDecimalDigits)
      ошибка(t.старт, ИДС.ВОктЧислеЕстьДесятичные);

    if (overflow)
      ошибка(t.старт, ИДС.ПереполнениеОктЧисла);
//     goto Lfinalize;
  ...
  проц  сканируйРеал(ref Сема t)
  {
    ...
    if (*p == 'e' || *p == 'E')
    {
      ...
      if (цифра_ли(*p))
        ...
        ошибка(t.старт, ИДС.ПлавЭкспДлжнНачСЦифры);
    }
    ...
    финализируйПлав(t, буфер);
  }

  /// Scans a hexadecimal floating point число literal.
  проц  сканируйГексРеал(ref Сема t)
  {
    assert(*p == '.' || *p == 'p' || *p == 'P');
    ИДС идс;
    if (*p == '.')
  ...
  проц  финализируйПлав(ref Сема t, ткст буфер)
  {
    assert(буфер[$-1] == 0);
    ...
    if (errno() == ERANGE)
      ошибка(t.старт, ИДС.ПереполнениеПлавЧисла);
    t.конец = p;
  }
  ...
  проц  сканируйОсобуйПоследовательностьСем(ref Сема t)
  {
    assert(*p == '#');
    t.вид = TOK.HashLine;
    t.установиФлагПробельные();
  ...
  дим раскодируйЮ8()
  {
    ...
    Lerr2:
      d = СИМ_ЗАМЕНЫ;
      ошибка(this.p, ИДС.НедействительнаяПоследовательностьУТФ8, форматируйБайты(this.p, p));
    }
  ...
  static ткст найдиНедействительнуюПоследовательностьУТФ8(ткст ткт)
  {
    ...
    while (p < конец)
    {
      if (раскодируй(p, конец) == СИМ_ОШИБКИ)
      {
        ...
        {}
        return Лексер.форматируйБайты(начало, p);
      }
    }
    assert(p == конец);
    return "";
...
unittest
{
  ...
  static Пара[] пары = [
    ...
    {">>>=",    TOK.БПСдвигПрисвой}, {"-=",      TOK.МинусПрисвой},
    {"<",       TOK.Меньше},          {"=",       TOK.Присвоить},
    {"<=",      TOK.МеньшеРавно},     {"==",      TOK.Равно},
    {"<>",      TOK.LorG},          {"~",       TOK.Тильда},
    {"<>=",     TOK.LorEorG},       {"~=",      TOK.КатПрисвой},
