...
class МодульМенеджер
{
  ...
  /// Мапирует ПКИ пути в модули. Напр.: drc/ast/узел
  Модуль[ткст] таблицаПКИПутейКМодулям;
  /// Карта абсолютных путей к файлам модулей. Напр.: /home/пользователь/drc/src/main.d
  ...
  /// Возвращает напр. 'drc.ast' для 'ddc/ast/узел'.
  ...
  static ткст найдиПутьКФайлуМодуля(ткст путьПоПКНМодуля, ткст[] путиИмпорта)
  {
    ...
    foreach (путьИмпорта; путиИмпорта)
    {
      ...
      путьКФайлу.добавь(путьПоПКНМодуля); // Напр.: drc/ast/узел
      foreach (суффиксМодуля; [".d", ".di"/*файл интерфейса*/])
      {
        путьКФайлу.суффикс(суффиксМодуля);
        if (путьКФайлу.есть_ли()) // Напр.: src/drc/ast/узел.d
