...
/// Таблица именованных сущностей ГЯР.
...
{
  бцел[] хэши; // Ткст хэши.
  дим[] значения; // Кодовые точки Юникод.
  // Построить массивы:
  foreach (сущность; именованныеСущности)
  {
    ...
    // Вставить хэш и значение в таблицы.
    if (i == хэши.length)
    {
      хэши ~= хэш;
      значения ~= значение;
    ...
    {
      хэши = хэши[0..i] ~ хэш ~ хэши[i..$]; // Вставить перед индексом.
      значения = значения[0..i] ~ значение ~ значения[i..$]; //Вставить перед индексом.
    }
    assert(хэши[i] == хэш && значения[i] == значение);
  }
  // Построить исходный текст:
...
/// Преобразует именованную сущность ГЯР в эквивалентную ей кодовую точку (Кодpoint) Юникод.
/// Возвращает: значение сущности или 0xFFFF, если её не есть_ли.
дим сущностьВЮникод(ткст сущность)
{
  ...
  while (нижний <= верхний)
  {
    ...
    if (хэш < хэши[индекс])
      ...
      return значения[индекс]; // Возвращает кодовую точку Юникод.
  }
  return 0xFFFF; // Возвращает значение ошибки.
}
