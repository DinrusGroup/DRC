...
ткст генерируйХэшИМассивЗначений()
{
  бцел[] хэши; // Ткст хэши.
  ...
  foreach (сущность; именованныеСущности)
  {
    ...
    for (; i < хэши.length; ++i)
    {
      assert(хэш != хэши[i], "плохая функция хэширования: конфликт хэшей");
      if (хэш < хэши[i])
        break;
    }
  ...
  // Build source текст:
  ткст текстХэшей = "private static const бцел[] хэши = [",
...
дим сущностьВЮникод(ткст сущность)
{
...
unittest
{
  выдай("Тестирую сущностьВЮникод().").нс;
