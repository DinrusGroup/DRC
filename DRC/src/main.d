...
проц  main(ткст[] арги)
{
  ...
  case "сем", "семанализ", "sem":
    ...
    {
      if (сема.вид == TOK.Новстр || игнорироватьШССемы && сема.пробел_ли)
        continue;
...
бул разборОтладкаИлиВерсия(ткст арг, КонтекстКомпиляции контекст)
{
  if (ткстнач(арг, "-отладка")||ткстнач(арг, "-debug"))
  {
    ...
    {
      auto знач = арг[7..$];
      if (drc.lexer.Funcs.цифра_ли(знач[0]))
        контекст.уровеньОтладки = Integer.вЦел(знач);
  ...
  else if (арг.length > 9 && ткстнач(арг, "-версия=")||ткстнач(арг, "-version="))
  {
    auto знач = арг[9..$];
    if (drc.lexer.Funcs.цифра_ли(знач[0]))
      контекст.уровеньВерсии = Integer.вЦел(знач);
    else if (Лексер.действитНерезИдентификатор_ли(знач))
...
проц  выведиСправку(ткст команда)
{
  ткст сооб;
	...
	Пример:
	  echo "module foo; проц  func(){}" | drc(дрк) лекс -
