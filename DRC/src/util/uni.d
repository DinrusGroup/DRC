...
цел униАльфа_ли(дим u)
{
	...
	{
	    assert(таблица[i][0] <= таблица[i][1]);
	    if (i < таблица.length - 1)
	    {
//		if (таблица[i][1] >= таблица[i + 1][0])
//		    printf("таблица[%d][1] = x%x, таблица[%d][0] = x%x\n", i, таблица[i][1], i + 1, таблица[i + 1][0]);
		assert(таблица[i][1] < таблица[i + 1][0]);
	    }
	}
    }
    ...
    low = 0;
    high = таблица.length - 1;
    while (cast(цел)low <= cast(цел)high)
    {
	идс = (low + high) >> 1;
	if (u < таблица[идс][0])
	    high = идс - 1;
	else if (u > таблица[идс][1])
	    low = идс + 1;
	else
	    goto Lis;
    }
...
Lisnot:
	...
	for (цел i = 0; i < таблица.length; i++)
	{
	    assert(u < таблица[i][0] || u > таблица[i][1]);
	}
    }
    return 0;
