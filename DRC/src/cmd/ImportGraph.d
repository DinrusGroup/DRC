...
struct ИКомандаГрафа
{
  ...
  проц  пуск()
  {
    ...
    auto граф = gbuilder.старт(путьКФайлу.имя());
  ...
  Вершина загрузиМодуль(ткст путьПоПКНМодуля)
  {
    ...
    { // Модуль not found.
      ...
      { // Include module nevertheless.
        ...
        вершина.модуль.установиПКН(замени(путьПоПКНМодуля, папРазд, '.'));
...
проц  выведиПутиКМодулям(Вершина[] вершины, бцел уровень, ткст отступ)
{
  ...
  {
    выдай(отступ)((вершина.цикличн_ли?"*":"")~вершина.модуль.путьКФайлу).нс;
...
проц  выведиСписокМодулей(Вершина[] вершины, бцел уровень, ткст отступ)
{
  ...
  foreach (вершина; вершины)
  {
    выдай(отступ)((вершина.цикличн_ли?"*":"")~вершина.модуль.дайПКН()).нс;
    if (вершина.исходящий.length)
                      ...
                      ИКомандаГрафа.Опции опции)
{
  ...
  foreach (i, вершина; граф.вершины)
    выдай.форматнс(`  n{} [лейбл="{}"{}];`, i, вершина.модуль.дайПКН(), (вершина.цикличн_ли ? ",style=filled,fillcolor=tomato" : ""));
