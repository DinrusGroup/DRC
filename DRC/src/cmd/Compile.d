...
struct КомандаКомпилировать
{
  ...
  проц  выведиДеревоМ(Пакет пкт, ткст отступ)
  {
    ...
    foreach (у; пкт.пакеты) // TODO: сортировать пакеты алфавитно по именам?
      выведиДеревоМ(у, отступ ~ "  ");
    foreach (m; пкт.модули) // TODO: сортировать модули алфавитно по именам?
  ...
  /// Импортирует модуль и запускает первую проходку по нему.
