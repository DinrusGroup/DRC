...
// Определить положение табулятора для вывода
#define AsmTab1  8                     // Столбец под опкод
#define AsmTab2  16                    // Столбец для первого операнда
#define AsmTab3  56                    // Столбец для комментария


// Структура для дефинирования x86 карт опкодов
struct SOpcodeDef {
   const char * Name;                  // название опкода
...
/****************     Константы для дефиниции опкодов     **********************
Я намеренно не присвоил имен этим константам, так как от этого таблицы в
opcodes.cpp были бы набиты множеством констант, связанных вместе посредством OR.
Было бы практически невозможно организовать столбцы для чтения.
Пардон, но вам придется просматривать константы в этом месте.

Следующие таблицы приводят возможное значение каждого из полей в SOpcodeDef:
...
InstructionSet:
(Некоторое значение можно присоеденить с помощью OR):
---------------------------
0:       8086
1:       80186
2:       80286
...
0x100:   8087
0x101:   80387
0x800:   Привилегированная инструкция
0x1001:  AMD 3DNow
0x1002:  Расширение AMD 3DNow
0x1004:  AMD SSE4a
0x1005:  AMD XOP
0x1006:  AMD FMA4
0x1007:  AMD CVT16
...
0x4000:  Доступно только в режиме 64 бит
0x8000:  Недоступно в режиме 64 бит
0x10000: Предложенный код инструкции, предварительная спецификация
0x20000: Предложенный код инструкции, но не реализованный,
         предварительная спецификация позднее была изменена

AllowedPrefixes:
(Значение можно присоединить посредством OR):
----------------------
