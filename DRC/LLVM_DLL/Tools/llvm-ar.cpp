...
ОПЦИИ:
  --format              - Формат создаваемого архива
    ...
    =darwin             -   darwin
    =bsd                -   bsd
  --plugin=<string>     - Игнорируется для совместимости
  --help                - Показать доступные опции
  --version             - Показать версию этой программы
...
static void getRelPos() {
  if (PositionalArgs.empty())
    fail("Ожидалась [релпоз] для модификатора a, b или i");
...
static void getCountParam() {
  if (PositionalArgs.empty())
    fail("Ожидался [счёт] для модификатора N");
  auto CountParamArg = StringRef(PositionalArgs[0]);
  if (CountParamArg.getAsInteger(10, CountParam))
    fail("Значение для [счёт] должно быть числовым, а получено: " + CountParamArg);
  if (CountParam < 1)
    fail("Значение для [счёт] должно быть положительным, а получено: " + CountParamArg);
  PositionalArgs.erase(PositionalArgs.begin());
}
...
static void getArchive() {
  if (PositionalArgs.empty())
    fail("Следует указать имя архива");
...
static object::Archive &readLibrary(const Twine &Library) {
  ...
  failIfError(BufOrErr.getError(), "Не удаётся открыть библиотеку " + Library);
  ArchiveBuffers.push_back(std::move(*BufOrErr));
  auto LibOrErr =
      object::Archive::create(ArchiveBuffers.back()->getMemBufferRef());
  failIfError(errorToErrorCode(LibOrErr.takeError()),
              "Не удалось разобрать библиотеку");
...
static ArchiveOperation parseCommandLine() {
  if (MRI) {
    if (!PositionalArgs.empty() || !Options.empty())
      fail("Не удалось смешать -M и другие опции");
    runMRIScript();
  }
  ...
  for (unsigned i = 0; i < Options.size(); ++i) {
    ...
    case 'L':
      ...
      break;
    default:
      fail(std::string("неизвестная опция ") + Options[i]);
    }
  }
  ...
  if (NumOperations == 0 && MaybeJustCreateSymTab) {
    ...
    if (!Members.empty())
      fail("Операция s в качестве аргумента принимает только архив");
  }

  // Perform various checks on the operation/modifier specification
  // to make sure we are dealing with a legal request.
  if (NumOperations == 0)
    fail("Следует указать хотя бы одну из операций");
  if (NumOperations > 1)
    fail("Следует указать только одну операцию");
  if (NumPositional > 1)
    fail("Следует указать только один модификатор из a, b и i");
  if (AddAfter || AddBefore)
    if (Operation != Move && Operation != ReplaceOrInsert)
      fail("Модификаторы 'a', 'b' и 'i' могут указываться только при "
           "операциях 'm' или 'r'");
  if (CountParam)
    if (Operation != Extract && Operation != Delete)
      fail("Модификатор 'N' может указываться только при  'x' или 'd' "
           "операциях");
  if (OriginalDates && Operation != Extract)
    fail("Модификатор 'o' применим только к операции 'x'");
  if (OnlyUpdate && Operation != ReplaceOrInsert)
    fail("Модификатор 'u' применим только к операции 'r'");
  if (AddLibrary && Operation != QuickAppend)
    fail("Модификатор 'L' применим только к операции 'q'");
...
static void performReadOperation(ArchiveOperation Operation,
  ...
  StringMap<int> MemberCount;
  {
    ...
    for (auto &C : OldArchive->children(Err)) {
      ...
      switch (Operation) {
      default:
        llvm_unreachable("Не операция чтения");
      case Print:
        doPrint(Name, C);
        break;
      case DisplayTable:
  ...
  for (StringRef Name : Members)
    WithColor::error(errs(), ToolName) << "'" << Name << "' не найден\n";
  exit(1);
}
...
static void addChildMember(std::vector<NewArchiveMember> &Members,
  ...
  if (Thin && !M.getParent()->isThin())
    fail("Не удаётся преобразовать обычный архив в тонкий");
...
static InsertAction computeInsertAction(ArchiveOperation Operation,
  ...
  llvm_unreachable("Такой операции нет");
}
...
static void performWriteOperation(ArchiveOperation Operation,
  ...
  case BSD:
    if (Thin)
      fail("Только формат gnu имеет тонкий режим");
...
static int performOperation(ArchiveOperation Operation,
  ...
  if (EC && EC != errc::no_such_file_or_directory)
    fail("ошибка при открытии '" + ArchiveName + "': " + EC.message() + "!");
  ...
  if (!EC) {
    ...
    failIfError(EC,
                "ошибка при загрузке '" + ArchiveName + "': " + EC.message() + "!");
    if (Archive.isThin())
      CompareFullPath = true;
  ...
  if (!shouldCreateArchive(Operation)) {
    failIfError(EC, Twine("ошибка при загрузке '") + ArchiveName + "'");
  } else {
    if (!Create) {
      // Produce a warning if we should and we're creating the archive
      WithColor::warning(errs(), ToolName)
          << "создаётся " << ArchiveName << "\n";
    }
  }
  ...
  for (line_iterator I(Ref, /*SkipBlanks*/ false), E; I != E; ++I) {
    ...
    case MRICommand::Create:
      ...
      if (!ArchiveName.empty())
        fail("Редактирование нескольких архивов не поддерживается");
      if (Saved)
        fail("Файл уже сохранён");
      ArchiveName = Rest;
      break;
    ...
    case MRICommand::Invalid:
      fail("Неизвестная команда: " + CommandStr);
    }
  }
...
extern "C" __declspec(dllexport) int ЛЛВхоФункцАр(char** args) {
  ...
  for (size_t i = 1; i < Argv.size(); ++i) {
    ...
    auto MatchFlagWithArg = [&](const char *expected) {
      ...
      if (Arg == expected) {
        if (++i >= Argv.size())
          fail(std::string(expected) + " требует аргумента");
        match = Argv[i];
        return true;
      }
    ...
    if (Arg[0] == '-') {
      ...
      if (Arg == "M") {
        ...
        if (FormatType == Unknown)
          fail(std::string("Неверный формат ") + match);
...
extern "C" __declspec(dllexport) int ЛЛВхоФункцРанлиб(char** args) {
    auto argn = (int)strlen((const char*)args);
    InitLLVM X(argn, args);
  ...
  for (int i = 1; i < argn; ++i) {
    if (handleGenericOption(args[i])) {
      ...
      if (ArchiveSpecified)
        fail("Надо указать ровно один архив");
...
extern "C" __declspec(dllexport) int ЛЛОбщВхоФункцЛЛАр( char **args) {
  ...
  if (Stem.contains_lower("dlltool"))
    return dlltoolDriverMain(makeArrayRef(args, argn));

  if (Stem.contains_lower("ranlib"))
    return ranlib_main(argn, args);

  if (Stem.contains_lower("lib"))
    return libDriverMain(makeArrayRef(args, argn));
